...
**香港中文大学（深圳）过港龙2026自瞄框架**



---

**本框架需配合视觉算法库 [DPRM](https://github.com/120090162/DPRM-2026) 食用**


**欢迎查看Wiki，获取更多信息 [自瞄算法Wiki](https://github.com/HHgzs/TJURM-2024/wiki/TJURM%E8%87%AA%E7%9E%84%E7%AE%97%E6%B3%95Wiki)**
...
- https://github.com/120090162/DPRM-2026
...
首先需要配置 DPRM 库，详见 [仓库](https://github.com/120090162/DPRM-2026) README。





### 编译安装 DPAUTOAIM-RM2026

成功编译和安装 DPRM 动态链接库后，DPAUTOAIM-RM2026 的环境依赖基本安装完毕了
...
由于本自瞄仓库为所有兵种共用，为避免参数等修改导致不同兵种间冲突，且为避免仓库频繁更新
 ...
 `dp2026_auto_aim/data/uniconfig` 下的参数信息将被安装到 `/etc/dprm` ，并建立名为 `dp2026_auto_aim/config` 的软链接到 `dp2026_auto_aim` 代码地址下

此时通过访问并修改 `dp2026_auto_aim/config` 内的参数便可直接更改系统中的参数，在下次代码运行时直接调用
...
- 可在项目文件夹 `dp2026_auto_aim` 内运行 `sudo ./run.sh -s`

  - 该方法将调用 `guard.sh` 守护脚本，保证代码在运行中终止时立即重启


- 可在任意地址下运行 `DPAUTOAIM-RM2026 -s`
...
在 DPRM 自瞄算法库中，实现了一个基于 `ncurses` 的终端参数面板，详见 **DPRM-2026** 仓库
...
- 如果没有 dprm 程序，请检查 DPRM 安装中是否忽略了安装此参数面板程序的步骤
- 详见 **DPRM-2026** 的 `README`   https://github.com/120090162/DPRM-2026
...
在参数面板左侧，有参数名称和参数值的元组，如果需要将某个或某几个参数以示波器的形式展示出来，可以使用 `dprm` 提供的示波器功能
...
```bash
dprm -o "target -x-" "target -y-" "target -z-" "target omg"
```
...
这里主要介绍 **Debug** 部分的参数功能。

需要修改`VideoSaveDir`和`DebugSaveDir`参数
...
"Debug" : {
        ...
        "SaveSpeed" : {                   // 英雄用弹速寄存器
            "SpeedWrite" : true,
            "SavePath" : "/etc/dprm/speed_log.txt"
        },

        "BigDecal" : "/etc/dprm/image/armor1.png",        // 重投影大装甲图像
        "SmallDecal" : "/etc/dprm/image/armor3.png"       // 重投影小装甲图像
    }
```
